...
class Player:
   ...
   def draw(self, screen):
      player_square = pygame.draw.rect(screen, self.color, pygame.Rect(self.position.x, self.position.y, self.size, self.size))
      center = Vector(player_square.centerx, player_square.centery)
      end_pos = (self.center.x + self.velocity.x * 50, self.center.y + self.velocity.y * 50)
