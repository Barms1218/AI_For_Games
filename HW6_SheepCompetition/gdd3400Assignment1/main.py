...
def buildGates(graph):
    ...
    for gate in PEN:
        ...
        graph.placeObstacle(
            Vector(gate[0][X] - Constants.GRID_SIZE, gate[0][Y]), (0, 255, 0))
        graph.placeObstacle(
            Vector(gate[0][X] - Constants.GRID_SIZE * 2, gate[0][Y]), (0, 255, 0))
        graph.placeObstacle(Vector(gate[1][X], gate[1][Y]), (255, 0, 0))
        graph.placeObstacle(
            Vector(gate[1][X] + Constants.GRID_SIZE, gate[1][Y]), (255, 0, 0))
        graph.placeObstacle(
            Vector(gate[1][X] + Constants.GRID_SIZE * 2, gate[1][Y]), (255, 0, 0))
    ...
    if finalGate[0][Y] == finalGate[1][Y]:
        ...
        for x in range(finalGate[0][X] + GRID_SIZE, finalGate[1][X], GRID_SIZE):
            graph.placeObstacle(
                Vector(x, finalGate[0][Y] + (PEN_DEPTH - GRID_SIZE)), (0, 0, 0))
    ...
    for y in range(gate[0][Y] + Constants.GRID_SIZE,
        ...
        graph.placeObstacle(
            Vector(gate[0][X] - Constants.GRID_SIZE, y), (0, 0, 0))
        graph.placeObstacle(
            Vector(gate[0][X] - Constants.GRID_SIZE * 2, y), (0, 0, 0))
        # Place the right side of the pen
        graph.placeObstacle(Vector(gate[1][X], y), (0, 0, 0))
        graph.placeObstacle(
            Vector(gate[1][X] + Constants.GRID_SIZE, y), (0, 0, 0))
        graph.placeObstacle(
            Vector(gate[1][X] + Constants.GRID_SIZE * 2, y), (0, 0, 0))
    ...
    for x in range(gate[0][X] + Constants.GRID_SIZE - 3 * Constants.GRID_SIZE,
                   ...
                   Constants.GRID_SIZE):
        graph.placeObstacle(
            Vector(x, gate[0][Y] + Constants.GRID_SIZE * 9), (0, 0, 0))
        graph.placeObstacle(
            Vector(x, gate[0][Y] + Constants.GRID_SIZE * 10), (0, 0, 0))
        graph.placeObstacle(
            Vector(x, gate[0][Y] + Constants.GRID_SIZE * 11), (0, 0, 0))
...
def buildObstacles(graph, penBounds):
    ...
    for i in range(Constants.NBR_RANDOM_OBSTACLES):
        ...
        start = Vector(randrange(0, Constants.WORLD_WIDTH),
...
while not hasQuit:
    ...
    text = font.render('%13s' % (' Time: ' + str(elapsedTime)
                                 ).ljust(13, ' '), True, (0, 255, 0), (0, 0, 255))
